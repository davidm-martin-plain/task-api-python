...
from src.tasks.infra.rest.dto.create_task_req import CreateTaskReq
from src.tasks.infra.task_config import get_task_adapter
from src.tasks.infra.rest.dto.task_res import TaskResponse
...
async def get_task(id: uuid.UUID, adapter: TaskPort = Depends(get_task_adapter)):
...
async def save_task(create_task_req: CreateTaskReq, adapter: TaskPort = Depends(get_task_adapter)):
    return TaskResponse.from_task(SaveTaskUseCase(create_task_req.to_task(), adapter).execute())

@router.delete("/{id}")
async def delete_task(id: uuid.UUID, adapter: TaskPort = Depends(get_task_adapter)) -> None:
